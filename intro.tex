...
The hats on the matrices such as $\hat{A}$, $\hamiltonian$ emphasize they play the roles of operators.
...
while the other useful one is the binary representation of computational basis $\ket{\vbx}=\ket{x_1,x_2,\dots,x_n},x_j\in \qty{0,1}$. 
...
($d-1$-dimensional) \emph{hyperplane} $(\vb{w},b)$ parametrized by a normal vector $\vb{w}\in\realnumber^n$ and a \emph{bias} term $b\in\realnumber$. in the (high-dimensional) \emph{feature space}.
...
% \begin{definition}[Positive semi-definite]\label{def:psd}
	A matrix $K\in\realnumber^{n\times n}$ is \emph{positive semi-definite} (PSD/p.s.d) if
	\begin{equation}
...
\begin{itemize}
	...
	Take $d=2,\nu=2$ as an example, the corresponding feature map $\Phi(\vbx)=(x_1^2,x_2^2,\sqrt{2c}x_1,\sqrt{2c}x_2,c)^\T$.
...
Discrete-time (DT) quantum random walk is the quantum analogy of classical DT random walk.
...
\end{equation}
% where $\omega_{\nu}=2(1-\cos\frac{2\pi \nu}{n})$.
\nameref{def:quantum_propagator} (CT quantum random walk/quantum kernel)
\begin{align}
...
An illustrative example is that quadratic speedup is optimal for unstructured (Grover) search \cite{groverQuantumMechanicsHelps1997} while exponential speedup is widely believed for integer (prime) factorization \cite{shorPolynomialTimeAlgorithmsPrime1997}.
...
physics datasets
\begin{itemize}
	\item particle physics: JET tagging task \cite{bogatskiyLorentzGroupEquivariant2020}; 
